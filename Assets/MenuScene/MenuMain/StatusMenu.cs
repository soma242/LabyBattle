...
public class StatusMenu : MonoBehaviour, IPointerClickHandler
{
    private Image image;
    ...
    private async UniTask MoveStatus()
    {
        holder.pointerPub.Publish(new MenuIPointerMessage());
        await UniTask.NextFrame();

        statusCanvas.enabled = true;
        var statePub = GlobalMessagePipe.GetPublisher<MainToStatusMessage>();
        statePub.Publish(new MainToStatusMessage());
