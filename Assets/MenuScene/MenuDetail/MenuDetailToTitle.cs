...
public class MenuDetailToTitle : MonoBehaviour, IPointerClickHandler
{
    private Image image;
    ...
    void Awake()
    {
        ...
        disposableOnDestroy = holder.selectSub.Subscribe(new SelectMessage(holder.detailLayer, DetailNum.titleNum), i =>
        {
            SelectThisComponent();
    ...
    private async UniTask SelectThisComponent()
    {
        ...
        holder.upSub.Subscribe(holder.detailLayer, i => {
            holder.selectPub.Publish(new SelectMessage(holder.detailLayer, DetailNum.quitNum), new SelectChange());
        }).AddTo(bag);
        ...
        holder.enterSub.Subscribe(holder.detailLayer, i => {
            ...
            Debug.Log("enter");
            var savePub = GlobalMessagePipe.GetPublisher<SaveCallMessage>();
            savePub.Publish(new SaveCallMessage());
            var titlePub = GlobalMessagePipe.GetPublisher<ToTitleMessage>();
            titlePub.Publish(new ToTitleMessage());
